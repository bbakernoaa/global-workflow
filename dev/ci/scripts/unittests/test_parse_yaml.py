...
template_value: "/path/to/homeglobal/some/path"''')
    ...
    def test_cli_default_value(self):
        ...
        cmd = [sys.executable, SCRIPT_PATH, "-y", TEST_CONFIG, "-k", "missing.key", "-d", "default_value"]
        result = subprocess.run(cmd, capture_output=True, text=True)
        self.assertEqual(result.returncode, 0, f"Command failed: {result.stderr}")
    ...
    def test_cli_fail_on_missing(self):
        ...
        cmd = [sys.executable, SCRIPT_PATH, "-y", TEST_CONFIG, "-k", "missing.key", "--fail-on-missing"]
        result = subprocess.run(cmd, capture_output=True, text=True)
    ...
    def test_cli_string_option(self):
        ...
        cmd = [sys.executable, SCRIPT_PATH, "-y", TEST_CONFIG, "-k", "list_data", "--string"]
        result = subprocess.run(cmd, capture_output=True, text=True)
        self.assertEqual(result.returncode, 0, f"Command failed: {result.stderr}")
