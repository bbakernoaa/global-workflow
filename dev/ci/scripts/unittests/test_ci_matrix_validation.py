...
class CIMatrixValidator:
    ...
    def extract_matrices_from_config(self) -> Dict[str, Set[str]]:
        ...
        """
        with open(self.gitlab_config_path) as f:
            content = f.read()
...
def test_detect_incorrectly_included_case(validator, repo_root_path):
    ...
    try:
        ...
        assert any(test_case_name in str(error) for error in errors[test_host]), f"Error should mention {test_case_name} case"
    finally:
...
def test_detect_missing_case(validator, repo_root_path):
    ...
    modified_content = re.sub(r"(\.hera_cases_matrix:.*?caseName: \[)(.*?)(\"C48_ATM\".*?\])", r"\1\3", original_content, flags=re.DOTALL)
    ...
    try:
        ...
        assert any("C48_ATM" in str(error) for error in errors["hera"]), "Error should mention C48_ATM case"
    finally:
