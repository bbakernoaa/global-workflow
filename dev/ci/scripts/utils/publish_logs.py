...
from githubpr import GitHubDBError, GitHubPR
...
def add_logs_to_gist(args, emcbot_gh):
    ...
    if args.multiple:
        ...
        for file in args.file:
            gist_files = {}
            with open(file.name, encoding="latin-1") as f:
                file_content = f.read()
            gist_files[os.path.basename(file.name)] = emcbot_gh.InputFileContent(file_content)

            gist = emcbot_gh.user.create_gist(
                public=True, files=gist_files, description=f"error log file from CI run {args.gist[0]} - {os.path.basename(file.name)}"
            )
...
def upload_logs_to_repo(args, emcbot_gh, emcbot_ci_url):
    ...
    while True:
        try:
            ...
            emcbot_gh.repo.get_contents(file_path_in_repo, ref="error_logs")
            path_header = f"{args.repo[0]}_{extra!s}"
        except GitHubDBError.GithubException:
            break

    for file in args.file:
        with open(file.name, encoding="latin-1") as file:
