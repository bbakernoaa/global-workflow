...
from deployment.workflow_config import DAG, TaskNode, parse
...
def _trace_execution_chain(dag: DAG, config_path: Path) -> ExecutionChain:
    ...
    with open(config_path) as f:
        _workflow_yaml = yaml.safe_load(f) or {}  # noqa: F841
...
def _deep_text(dag: DAG, chain: ExecutionChain) -> str:
    ...
    for family_path, nodes in sorted(families.items()):
        ...
        lines.append(f"  {'─' * 60}")
        for node in sorted(nodes, key=lambda n: n.name):
            app_name, source_name = chain.task_to_jjob.get(node.full_path, (node.jjob, node.jjob))
            rename_note = ""
            ...
            if ex_scripts:
                ...
                lines.append("          → (no ex-scripts found)")
            lines.append("")
...
def _deep_mermaid(dag: DAG, chain: ExecutionChain, direction: str = "LR") -> str:
    ...
    # Emit J-Job nodes
    lines.append('    subgraph jjobs["J-Jobs"]')
    ...
    if all_ex:
        lines.append('    subgraph scripts["Ex-Scripts"]')
    ...
    if all_ush:
        lines.append('    subgraph ush_scripts["Ush Scripts"]')
...
def _deep_dot(dag: DAG, chain: ExecutionChain) -> str:
    ...
    for family_name, nodes in sorted(families.items()):
        ...
        lines.append('        style=filled; color="#e8f4fd";')
    ...
    # J-Job nodes
    lines.append("    subgraph cluster_jjobs {")
    lines.append('        label="J-Jobs (dev/jobs/)";')
    lines.append('        style=filled; color="#fde8e8";')
    lines.append('        node [shape=box, style="filled,bold", fillcolor="#fff0f0"];')
    for source_name in sorted(chain.jjob_to_ex_scripts.keys()):
    ...
    lines.append('        label="Ex-Scripts (dev/scripts/)";')
    lines.append('        style=filled; color="#e8fde8";')
    lines.append('        node [shape=ellipse, style=filled, fillcolor="#f0fff0"];')
    for ex in sorted(all_ex):
        eid = _dot_id(f"ex:{ex}")
    ...
    for ush_set in chain.ex_to_ush.values():
        all_ush.update(ush_set)
    if all_ush:
        lines.append("    subgraph cluster_ush {")
        lines.append('        label="Ush Scripts (dev/ush/)";')
        lines.append('        style=filled; color="#fdf8e8";')
        lines.append('        node [shape=parallelogram, style=filled, fillcolor="#fffff0"];')
        for ush in sorted(all_ush):
