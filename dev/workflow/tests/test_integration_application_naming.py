...
class TestEndToEndApplicationNaming:
    ...
    def test_full_pipeline_produces_application_named_expdir(self, project_root: Path, expdir: Path):
        ...
        assert resolution_map["JGCDAS_AERO_ANALYSIS_GENERATE_BMATRIX"].source_name == ("JGDAS_AERO_ANALYSIS_GENERATE_BMATRIX")
        ...
        assert scan_result.passed, f"EE2 scan found {len(scan_result.violations)} violation(s):\n" + "\n".join(
            v.format() for v in scan_result.violations
        )
        ...
        for filename in staged_files:
            assert not filename.startswith("JGLOBAL_"), f"Found shared-name file '{filename}' in EXPDIR — expected only application-named files"
        ...
        assert staged_content == source_content, "Content mismatch: staged application-named file differs from source"
        ...
        assert mode == 0o755, f"Expected 0755 permissions on link_workflow.sh, got {oct(mode)}"

        link_fixdirs = expdir / "sorc" / "ufs_utils.fd" / "fix" / "link_fixdirs.sh"
        assert link_fixdirs.exists(), "sorc/ufs_utils.fd/fix/link_fixdirs.sh not found in EXPDIR"
        assert link_fixdirs.read_text() == _LINK_FIXDIRS_CONTENT
        mode = stat.S_IMODE(link_fixdirs.stat().st_mode)
        assert mode == 0o755, f"Expected 0755 permissions on link_fixdirs.sh, got {oct(mode)}"

    def test_ee2_scan_passes_on_application_named_expdir(self, project_root: Path, expdir: Path):
        ...
        # Scan
        scan_result = scan_expdir(expdir)
        assert scan_result.passed, f"EE2 scan should pass on application-named J-Jobs.\nViolations: {[v.format() for v in scan_result.violations]}"

    def test_linking_scripts_staged_with_executable_permissions(self, project_root: Path, expdir: Path):
...
class TestBackwardCompatibilityIntegration:
    ...
    def test_shared_name_yaml_no_rename(self, project_root: Path, expdir: Path):
        ...
        for name in shared_names:
            assert resolution_map[name].is_passthrough is True, f"Shared name '{name}' should be pass-through"
            assert resolution_map[name].source_name == name, f"Shared name '{name}' source should equal itself"
        ...
        for name in shared_names:
            ...
            assert staged_content == source_content, f"Content mismatch for passthrough file '{name}'"
    ...
    def test_ee2_scan_passes_on_shared_named_expdir(self, project_root: Path, expdir: Path):
        ...
        scan_result = scan_expdir(expdir)
        assert scan_result.passed, f"EE2 scan should pass on shared-name J-Jobs.\nViolations: {[v.format() for v in scan_result.violations]}"
