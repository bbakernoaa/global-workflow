...
def _create_minimal_dev_tree(base_path: Path) -> dict:
    ...
    (env_dir / "HERA.env").write_text("#!/bin/bash\n# HERA environment\nexport MACHINE=HERA\nexport SCHEDULER=slurm\nexport PARTITION=hera\n")
    (env_dir / "WCOSS2.env").write_text("#!/bin/bash\n# WCOSS2 environment\nexport MACHINE=WCOSS2\nexport SCHEDULER=pbs\nexport QUEUE=dev\n")
    (env_dir / "ORION.env").write_text("#!/bin/bash\n# ORION environment\nexport MACHINE=ORION\nexport SCHEDULER=slurm\nexport PARTITION=orion\n")
    ...
    (jobs_dir / "JGLOBAL_STAGE_IC").write_text(jjob_template.format(description="Stage initial conditions"))
    (jobs_dir / "JGLOBAL_FORECAST").write_text(jjob_template.format(description="Run GFS forecast"))
    (jobs_dir / "JGFS_ATMOS_POST").write_text(jjob_template.format(description="Post-processing"))
    (jobs_dir / "JGLOBAL_ARCHIVE").write_text(jjob_template.format(description="Archive products"))

    # --- Ex-scripts (platform-independent) ---
    scripts_dir = dev_root / "scripts"
    scripts_dir.mkdir(exist_ok=True)
    (scripts_dir / "exglobal_stage_ic.sh").write_text("#!/bin/bash\n# Ex-script: Stage IC\necho 'stage_ic done'\n")
    ...
    (ush_dir / "detect_machine.sh").write_text("#!/bin/bash\n# Detect the current HPC platform\necho ${MACHINE:-UNKNOWN}\n")
    (ush_dir / "err_exit.sh").write_text("#!/bin/bash\n# Error exit handler\nexit 1\n")
    ...
    (gfs_config / "config.resources.HERCULES").write_text("#!/bin/bash\n# HERCULES-specific resources\nexport ntasks=80\nexport partition=hercules\n")
    ...
    (modulefiles_dir / "gw_setup.orion.lua").write_text('-- Orion setup module\nload("cmake/3.22")\n')
    (modulefiles_dir / "gw_run.hercules.lua").write_text('-- Hercules run module\nload("intel/2023.1")\n')
    (modulefiles_dir / "gw_setup.hercules.lua").write_text('-- Hercules setup module\nload("cmake/3.25")\n')
    (modulefiles_dir / "gw_run.common.lua").write_text("-- Common run module (shared)\n")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            assert not unexpected_diffs, (
                ...
                + "\n".join(f"  {path}: {diff_type}" for path, diff_type in sorted(unexpected_diffs.items()))
            )

            # Additional assertion: J-Jobs are identical
            jobs_a = {k: v for k, v in files_a.items() if k.startswith("jobs/")}
            jobs_b = {k: v for k, v in files_b.items() if k.startswith("jobs/")}
            assert jobs_a == jobs_b, f"J-Jobs differ between {platform_a} and {platform_b}! J-Jobs must be platform-independent."

            # Additional assertion: ex-scripts are identical
            scripts_a = {k: v for k, v in files_a.items() if k.startswith("scripts/")}
            scripts_b = {k: v for k, v in files_b.items() if k.startswith("scripts/")}
            assert scripts_a == scripts_b, f"Ex-scripts differ between {platform_a} and {platform_b}! Ex-scripts must be platform-independent."

            # Additional assertion: ush utilities are identical
            ush_a = {k: v for k, v in files_a.items() if k.startswith("ush/")}
            ush_b = {k: v for k, v in files_b.items() if k.startswith("ush/")}
            assert ush_a == ush_b, f"ush utilities differ between {platform_a} and {platform_b}! ush utilities must be platform-independent."

            # Assert that platform-conditioned differences DO exist
            # (confirms the platforms actually produce different content)
            assert len(diffs) > 0, (
                f"No differences found between {platform_a} and {platform_b} deployments. Expected at least env/ and modulefiles/ to differ."
            )
...
class TestPlatformIsolationHERAvsWCOSS2:
    ...
    def test_jobs_identical(self, deployed_pair):
        ...
        jobs_hera = {k: v for k, v in deployed_pair["files_hera"].items() if k.startswith("jobs/")}
        jobs_wcoss2 = {k: v for k, v in deployed_pair["files_wcoss2"].items() if k.startswith("jobs/")}
        assert jobs_hera == jobs_wcoss2

    def test_scripts_identical(self, deployed_pair):
        """Ex-scripts are identical across HERA and WCOSS2."""
        scripts_hera = {k: v for k, v in deployed_pair["files_hera"].items() if k.startswith("scripts/")}
        scripts_wcoss2 = {k: v for k, v in deployed_pair["files_wcoss2"].items() if k.startswith("scripts/")}
    ...
    def test_ush_identical(self, deployed_pair):
        ...
        ush_hera = {k: v for k, v in deployed_pair["files_hera"].items() if k.startswith("ush/")}
        ush_wcoss2 = {k: v for k, v in deployed_pair["files_wcoss2"].items() if k.startswith("ush/")}
        assert ush_hera == ush_wcoss2

    def test_env_files_differ(self, deployed_pair):
        """env/ files differ between HERA and WCOSS2."""
        env_hera = {k: v for k, v in deployed_pair["files_hera"].items() if k.startswith("env/")}
        env_wcoss2 = {k: v for k, v in deployed_pair["files_wcoss2"].items() if k.startswith("env/")}
    ...
    def test_modulefiles_differ(self, deployed_pair):
        ...
        mods_hera = {k: v for k, v in deployed_pair["files_hera"].items() if k.startswith("modulefiles/")}
        mods_wcoss2 = {k: v for k, v in deployed_pair["files_wcoss2"].items() if k.startswith("modulefiles/")}
    ...
    def test_ecf_scripts_differ(self, deployed_pair):
        ...
        ecf_hera = {k: v for k, v in deployed_pair["files_hera"].items() if k.endswith(".ecf")}
        ecf_wcoss2 = {k: v for k, v in deployed_pair["files_wcoss2"].items() if k.endswith(".ecf")}
        # Same set of .ecf files should exist
        assert set(ecf_hera.keys()) == set(ecf_wcoss2.keys()), "Different .ecf files generated for HERA vs WCOSS2"
        # But content should differ (scheduler directives)
        any_differ = any(ecf_hera[k] != ecf_wcoss2[k] for k in ecf_hera)
        assert any_differ, ".ecf scripts should differ between HERA (Slurm) and WCOSS2 (PBS)"
    ...
    def test_only_platform_conditioned_paths_differ(self, deployed_pair):
        ...
        unexpected = {path: diff_type for path, diff_type in diffs.items() if not _is_platform_conditioned(path, "HERA", "WCOSS2")}

        assert not unexpected, "Unexpected differences between HERA and WCOSS2:\n" + "\n".join(f"  {p}: {d}" for p, d in sorted(unexpected.items()))
