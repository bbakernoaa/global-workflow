...
class ModelContextSchema:
    ...
    required_top_level_keys: list[str] = field(default_factory=lambda: list(REQUIRED_TOP_LEVEL_KEYS))
    required_fv3_keys: list[str] = field(default_factory=lambda: list(REQUIRED_FV3_KEYS))
    required_aerosol_keys: list[str] = field(default_factory=lambda: list(REQUIRED_AEROSOL_KEYS))
    ...
    def validate(self, model_context: dict[str, Any]) -> list[str]:
        ...
        for key in self.required_top_level_keys:
            if key not in model_context:
                errors.append(f"FATAL ERROR: Missing required key 'model.{key}' in Model_Context")

        # Validate top-level value constraints
        if "resolution" in model_context:
            resolution = model_context["resolution"]
            if resolution not in SUPPORTED_RESOLUTIONS:
                errors.append(f"FATAL ERROR: Unsupported resolution '{resolution}'. Supported values: {sorted(SUPPORTED_RESOLUTIONS)}")
        ...
        if "physics_suite" in model_context:
            ...
            if physics_suite not in SUPPORTED_PHYSICS_SUITES:
                errors.append(f"FATAL ERROR: Unsupported physics_suite '{physics_suite}'. Supported values: {sorted(SUPPORTED_PHYSICS_SUITES)}")
        ...
        if "coupling_mode" in model_context:
            ...
            if coupling_mode not in SUPPORTED_COUPLING_MODES:
                errors.append(f"FATAL ERROR: Unsupported coupling_mode '{coupling_mode}'. Supported values: {sorted(SUPPORTED_COUPLING_MODES)}")

        if "dt_atmos" in model_context:
            dt_atmos = model_context["dt_atmos"]
            if not isinstance(dt_atmos, int) or dt_atmos <= 0:
                errors.append(f"FATAL ERROR: 'model.dt_atmos' must be a positive integer, got {dt_atmos!r}")
    ...
    def _validate_fv3(self, fv3: dict[str, Any]) -> list[str]:
        ...
        for key in self.required_fv3_keys:
            if key not in fv3:
                errors.append(f"FATAL ERROR: Missing required key 'model.fv3.{key}' in Model_Context")

        # Type validations for present keys
        if "npx" in fv3:
            if not isinstance(fv3["npx"], int) or fv3["npx"] <= 0:
                errors.append(f"FATAL ERROR: 'model.fv3.npx' must be a positive integer, got {fv3['npx']!r}")

        if "npy" in fv3:
            if not isinstance(fv3["npy"], int) or fv3["npy"] <= 0:
                errors.append(f"FATAL ERROR: 'model.fv3.npy' must be a positive integer, got {fv3['npy']!r}")

        if "npz" in fv3:
            if not isinstance(fv3["npz"], int) or fv3["npz"] <= 0:
                errors.append(f"FATAL ERROR: 'model.fv3.npz' must be a positive integer, got {fv3['npz']!r}")

        if "layout" in fv3:
            layout = fv3["layout"]
            if not isinstance(layout, (list, tuple)) or len(layout) != 2 or not all(isinstance(x, int) and x > 0 for x in layout):
                errors.append(f"FATAL ERROR: 'model.fv3.layout' must be a two-element list of positive integers, got {layout!r}")

        if "io_layout" in fv3:
            io_layout = fv3["io_layout"]
            if not isinstance(io_layout, (list, tuple)) or len(io_layout) != 2 or not all(isinstance(x, int) and x >= 0 for x in io_layout):
                errors.append(f"FATAL ERROR: 'model.fv3.io_layout' must be a two-element list of non-negative integers, got {io_layout!r}")

        if "quilting" in fv3:
            if not isinstance(fv3["quilting"], bool):
                errors.append(f"FATAL ERROR: 'model.fv3.quilting' must be a boolean, got {fv3['quilting']!r}")

        if "write_group" in fv3:
            if not isinstance(fv3["write_group"], int) or fv3["write_group"] <= 0:
                errors.append(f"FATAL ERROR: 'model.fv3.write_group' must be a positive integer, got {fv3['write_group']!r}")

        if "wrttask_per_group" in fv3:
            if not isinstance(fv3["wrttask_per_group"], int) or fv3["wrttask_per_group"] <= 0:
                errors.append(f"FATAL ERROR: 'model.fv3.wrttask_per_group' must be a positive integer, got {fv3['wrttask_per_group']!r}")

        if "restart_interval" in fv3:
            if not isinstance(fv3["restart_interval"], int) or fv3["restart_interval"] < 0:
                errors.append(f"FATAL ERROR: 'model.fv3.restart_interval' must be a non-negative integer, got {fv3['restart_interval']!r}")
    ...
    def _validate_aerosol(self, aerosol: dict[str, Any]) -> list[str]:
        ...
        for key in self.required_aerosol_keys:
            if key not in aerosol:
                errors.append(f"FATAL ERROR: Missing required key 'model.aerosol.{key}' in Model_Context")
        ...
        if "emission_dataset" in aerosol:
            ...
            if emission_dataset not in SUPPORTED_EMISSION_DATASETS:
                errors.append(
                    f"FATAL ERROR: Unsupported emission_dataset '{emission_dataset}'. Supported values: {sorted(SUPPORTED_EMISSION_DATASETS)}"
                )
        ...
        if "active_collections" in aerosol:
            ...
            if not isinstance(collections, list) or len(collections) == 0:
                errors.append(f"FATAL ERROR: 'model.aerosol.active_collections' must be a non-empty list, got {collections!r}")

        if "grid_label" in aerosol:
            grid_label = aerosol["grid_label"]
            if not isinstance(grid_label, str) or not grid_label:
                errors.append(f"FATAL ERROR: 'model.aerosol.grid_label' must be a non-empty string, got {grid_label!r}")
...
def validate_coupled_model_context(model_context: dict[str, Any]) -> list[str]:
    ...
    for section, keys in COUPLED_REQUIRED_KEYS.items():
        ...
        if section_data is None:
            errors.append(f"FATAL ERROR: Missing required section 'model.{section}'")
            continue
        for key in keys:
            if key not in section_data:
                errors.append(f"FATAL ERROR: Missing required key 'model.{section}.{key}'")
    ...
    if ocean is not None and "resolution" in ocean:
        ...
        if resolution not in SUPPORTED_OCEAN_RESOLUTIONS:
            errors.append(f"FATAL ERROR: Invalid ocean.resolution '{resolution}'. Must be one of: {', '.join(sorted(SUPPORTED_OCEAN_RESOLUTIONS))}")
    ...
    if wave is not None:
        if "ice_input" in wave:
            ...
            if ice_input not in SUPPORTED_WAVE_ICE_INPUT:
                errors.append(f"FATAL ERROR: Invalid wave.ice_input '{ice_input}'. Must be one of: {', '.join(sorted(SUPPORTED_WAVE_ICE_INPUT))}")
        if "current_input" in wave:
            current_input = wave["current_input"]
            if current_input not in SUPPORTED_WAVE_CURRENT_INPUT:
                errors.append(
                    f"FATAL ERROR: Invalid wave.current_input '{current_input}'. Must be one of: {', '.join(sorted(SUPPORTED_WAVE_CURRENT_INPUT))}"
                )
    ...
    if post is not None and "system" in post:
        ...
        if system not in SUPPORTED_POST_SYSTEMS:
            errors.append(f"FATAL ERROR: Invalid post.system '{system}'. Must be one of: {', '.join(sorted(SUPPORTED_POST_SYSTEMS))}")
...
def merge_ocean_resolution_defaults(model_context: dict[str, Any]) -> dict[str, Any]:
    ...
    if resolution not in SUPPORTED_OCEAN_RESOLUTIONS:
        raise FatalDeploymentError(f"Unsupported ocean resolution '{resolution}'. Supported: 025, 050, 100, 500")
