...
from wxflow import AttrDict, Executable, FileHandler, WorkflowException, jinja, logit, parse_j2yaml, save_as_yaml
...
@logit(logger)
def prep_input_nml(task_config: AttrDict, output_nml: str = "mom_input.nml", simple_geom: bool = False, mom_input: str = "MOM_input") -> None:
...
def test_hist_date(histfile: str, ref_date: datetime) -> None:
    ...
    if hist_date != ref_date:
        raise ValueError("FATAL ERROR: Inconsistent bkg date'")
...
def gen_bkg_list(bkg_path: str, window_begin=" ", ice_rst=False) -> None:
    ...
    # ocn_filename_ic = "./INPUT/MOM.res.nc"  # reserved for future vertical remapping
    ...
    for bkg in files:
        ...
        bkg_dict = {
            "date": bkg_date.strftime("%Y-%m-%dT%H:%M:%SZ"),
            "basename": "./bkg/",
...
def get_mom6_levels(ocnres: str) -> int:
    ...
    ocnres_to_nlev = {"500": 25, "100": 75, "050": 75, "025": 75}
    try:
