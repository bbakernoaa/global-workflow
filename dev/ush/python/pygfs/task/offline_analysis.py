...
from wxflow import AttrDict, Executable, FileHandler, Task, WorkflowException, logit
...
class OfflineAnalysis(Task):
    ...
    def initialize(self) -> None:
        ...
        files_to_copy = []
        fcst_file_in = os.path.join(self.task_config.COMIN_ATMOS_HISTORY_PREV, f"{self.task_config.GPREFIX}atm.f006.nc")
        files_to_copy.append([fcst_file_in, os.path.join(self.task_config.DATA, "atmges_mem001")])
        sfcfcst_file_in = os.path.join(self.task_config.COMIN_ATMOS_HISTORY_PREV, f"{self.task_config.GPREFIX}sfc.f006.nc")
        ...
        namelist = {
            ...
            "zeroinc": {"incvars_to_zero": self.task_config.INCREMENTS_TO_ZERO},
        }
        logger.info(namelist)
        ...
        for exec_name in executable_list:
            executables_to_copy.append([os.path.join(self.task_config.EXECglobal, exec_name), os.path.join(self.task_config.DATA, exec_name)])
    ...
    def finalize(self) -> None:
        ...
        transfer_files.append(
            [
                os.path.join(self.task_config.COMINobsproc, f"{self.task_config.APREFIX_IN}rtgssthr.grb"),
                os.path.join(self.task_config.COMOUT_OBS, f"{self.task_config.APREFIX}rtgssthr.grb"),
            ]
        )
        transfer_files.append(
            [
                os.path.join(self.task_config.COMINobsproc, f"{self.task_config.APREFIX_IN}seaice.5min.blend.grb"),
                os.path.join(self.task_config.COMOUT_OBS, f"{self.task_config.APREFIX}seaice.5min.blend.grb"),
            ]
        )
        transfer_files.append(
            [
                os.path.join(self.task_config.COMINobsproc, f"{self.task_config.APREFIX_IN}snogrb_t1534.3072.1536"),
        ...
        transfer_files.append(
            [
                ...
                os.path.join(self.task_config.COMOUT_ATMOS_ANALYSIS, f"{self.task_config.APREFIX}increment.dtf.i006.nc"),
            ]
        )
