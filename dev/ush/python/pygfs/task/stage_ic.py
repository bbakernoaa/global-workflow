...
from wxflow import AttrDict, FileHandler, Task, add_to_datetime, logit, parse_j2yaml, to_fv3time, to_timedelta, to_YMD
...
class Stage(Task):
    ...
    def _copy_com_templates(self) -> Dict[str, str]:
        ...
        """
        return {key: self.task_config[key] for key in self.task_config.keys() if key.startswith("COM_") and key.endswith("_TMPL")}
    ...
    def create_stage_dict(self) -> AttrDict:
        ...
        stage_dict["member_list"] = self.get_member_list(
            stage_dict.RUN, stage_dict.NMEM_ENS, m_index=stage_dict.get("m_index", 0), gefstype=stage_dict.get("GEFSTYPE", None)
        )
