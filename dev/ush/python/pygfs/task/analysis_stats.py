...
class AnalysisStats(Analysis):
    ...
    def initialize(self, analysis: str) -> None:
        ...
        diag_archive = os.path.join(jcb_config[f"{component}_obsdatain_path"], f"{self.task_config.APREFIX}{analysis}_analysis.ioda_hofx.tar.gz")
        Jedi.extract_tar(diag_archive)

        # Initialize JEDI application
        logger.info("Initializing JEDI ioda-stats extraction application")
    ...
    def finalize(self, analysis: str) -> None:
        ...
        self.jedi_dict[analysis].save_obsdataout(outdir, f"{self.task_config.APREFIX}{analysis}_analysis.ioda_hofx_stats")
    ...
    def convert_gsi_diags(self) -> None:
        ...
        for diag in diag_tars:
            ...
            input_tar = os.path.join(self.task_config.COMIN_ATMOS_ANALYSIS, input_tar_basename)
        ...
        iodastatzipfile = os.path.join(
            self.task_config.DATA, "atmos_gsi", "atmos_gsi_ioda", f"{self.task_config.APREFIX}atmos_gsi_analysis.ioda_hofx.tar.gz"
        )
